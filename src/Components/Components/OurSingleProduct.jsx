...
const OurSingleProduct = () => {
  ...
  // here extract id from object and then show detial of a single product
  const foundProduct = findProduct.apply(null, [Number(id)]);
    ...
    <div className="mx-auto w-11/12">
        ...
        <div className="sm:w-1/2 w-full flex items-center">
          <div className={`2/3  `}>
            <img
              ...
              src={mainImage || foundProduct.image1}
              alt={foundProduct.name}
            />
          </div>
          <div className=" justify-between w-1/3 ml-4">
            <img
              onClick={() => handleMainImage(foundProduct.image1)}
              className="w-32 mb-4 cursor-pointer"
              src={foundProduct.image1}
              alt={foundProduct.name}
            />
            <img
              onClick={() => handleMainImage(foundProduct.image2)}
              className="w-32 mb-4 cursor-pointer"
              src={foundProduct.image2}
              alt={foundProduct.name}
            />
            <img
              onClick={() => handleMainImage(foundProduct.image3)}
              className="w-32 mb-4 cursor-pointer"
              src={foundProduct.image3}
              alt={foundProduct.name}
            />
          </div>
        </div>
        <div className="sm:w-1/2 w-full">
          <h1 className=" text-3xl font-bold">{foundProduct.productName}</h1>
          ...
          <div className="flex font-bold py-2">
            ...
            <div className="pl-2 font-medium">
              {foundProduct.availability <= 5 ? (
                <span>Low stock: {foundProduct.availability} Left</span>
              ) : (
                <span>In stock: {foundProduct.availability} Left</span>
              )}
            </div>
          </div>
          <div className="flex items-center py-2">
            <p className="font-bold text-2xl">${foundProduct.oldPrice}.00</p>
            <s className="font-medium text-xl text-gray-500 pl-2">
              ${foundProduct.price}.00
            </s>
          ...
          <div className="my-5">
            <button
              onClick={() => handleAddToCart(foundProduct, foundProduct.id)}
              className="bg-[#73841B]  text-white hover:bg-black py-2 px-7  rounded-full"
            >
              Add to Cart
            </button>
          ...
          <div className="my-5">
            <p className="font-bold text-xl">
              SKU:{" "}
              <span className="text-lg font-normal">{foundProduct.sku}</span>
            </p>
          </div>
      ...
      <div className="md:my-10 my-5">
        ...
        {activeButton === "reviews" && (
          <div className="md:my-7 my-4">review Here</div>
        )}
      </div>
    </div>
  );
